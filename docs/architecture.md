...
4. The API creates a new OTP record with expiry metadata and supersedes previous active records for that identifier.
5. The frontend displays the request result and prompts for the OTP.
6. If the user requests a resend, the API checks resend limits and creates the next latest OTP state.
...
9. On successful verification, the API marks the OTP as used with a conditional or transactional write and returns a success response.
